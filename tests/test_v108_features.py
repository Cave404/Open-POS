...
def test_v108_version_milestone(client):
    ...
    assert Config.VERSION in ("v1.0.8", "v1.0.9")

    res_credits = client.get("/api/system/credits")
    assert res_credits.status_code == 200
    assert res_credits.get_json()["version"] in ("v1.0.8", "v1.0.9")

    # Verify templates render v1.0.8 or v1.0.9
    routes = [
        "/manager",
        "/manager/about",
        "/manager/branding",
    ...
    for route in routes:
        ...
        assert any(v in res.get_data(as_text=True) for v in ("v1.0.8", "v1.0.9")), \
